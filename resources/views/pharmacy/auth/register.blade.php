        ...
        <div class="max-w-md w-full space-y-8 bg-white p-10 rounded-2xl shadow-2xl text-right">
            ...
            <div>
                ...
                <p class="mt-2 text-center text-sm text-gray-600">
                    هل لديك حساب بالفعل؟
                    <a href="{{ route('pharmacy.login') }}" class="font-medium text-indigo-600 hover:text-indigo-500">
                        سجل الدخول هنا
                    </a>
                </p>
            </div>
            ...
            <!-- النموذج -->
            <form method="POST" action="{{ route('pharmacy.register') }}" enctype="multipart/form-data" class="mt-8 space-y-6">
                @csrf
                ...
                <!-- العنوان -->
                <div>
                    <label for="address" class="block text-sm font-medium text-gray-700">عنوان الصيدلية</label>
                    <input id="address" name="address" type="text"  dir="rtl" required
                           class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                           value="{{ old('address') }}">
                    @error('address')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                ...
                <!-- الوصف -->
                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700">وصف الصيدلية (اختياري)</label>
                    <textarea id="description" name="description"
                              class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('description') }}</textarea>
                    @error('description')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- تحميل الصورة -->
                <div>
                    <label for="image" class="block text-sm font-medium text-gray-700">صورة الصيدلية </label>
